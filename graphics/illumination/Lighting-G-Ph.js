...
function startWebGL() {
	...
	shaderProgram = createShadersPrograms();
...
function drawloop(now) {
	...
	gl.uniformMatrix4fv(gl.getUniformLocation(shaderProgram, 'uViewMatrix'), false, Camera.cameraTransformationMatrix);	
...
function drawObj(bufs, obj, x, y, z, xrotate, yrotate, zrotate) {
	...
	const normCoord = gl.getAttribLocation(shaderProgram, "normalVectors");
	gl.vertexAttribPointer(
	...
	// const texCoord = gl.getAttribLocation(shaderProgram, "textureCoords");
	// gl.vertexAttribPointer(
	...
	gl.uniform4fv(gl.getUniformLocation(shaderProgram, 'uShapeColor'), vec4.fromValues(0/255, 0/255, 255/255, 255/255));
	gl.uniform3fv(gl.getUniformLocation(shaderProgram, 'uDirectionalLightColor'), vec3.fromValues(1.0, 1.0, 1.0));
	gl.uniform3fv(gl.getUniformLocation(shaderProgram, 'uDirectionalLightVector'), vec3.fromValues(dirSourceX, dirSourceY, dirSourceZ));
	gl.uniform3fv(gl.getUniformLocation(shaderProgram, 'uAmbientLightColor'), vec3.fromValues(ambientLightColor/255, ambientLightColor/255, ambientLightColor/255));

	gl.uniform1f(gl.getUniformLocation(shaderProgram, 'shininess'), shininess);

	gl.uniform4fv(gl.getUniformLocation(shaderProgram, 'specColor'), vec4.fromValues(1,1,1,1));
	gl.uniform4fv(gl.getUniformLocation(shaderProgram, 'specMatColor'), vec4.fromValues(1,1,1,1));
